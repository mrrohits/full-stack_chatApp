...
```bash
git clone  https://github.com/RohitRawat891997/full-stack_chatApp.git
```
...
```bash
docker build -t chatapp_frontend:v1 .
```

### Run the Frontend container:

```bash
docker run -d --network=full-stack  -p 5173:5173 --name=frontend chatapp_frontend:v1
```
...
```bash
docker build -t chatapp_backend:v1 .
```

### Run the Backend container:

```bash
docker run -d --network=full-stack --add-host=host.docker.internal:host-gateway -p 5001:5001 --env-file .env chatapp_backend:v1
